...
ENV VIRTUAL_ENV /opt/cajudsa-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/cajudsa/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/cajudsa/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/cajudsa/ && touch src/cajudsa/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/cajudsa/
